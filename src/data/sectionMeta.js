...
// DocCategoryGeneratedIndexPage, the homepage, and the Learning Path page.
// Time estimates are an honest range derived from page count x a realistic
// 15-25 min/page for this site's technical density -- not a false-precision
// single number, and not a fixed weekly cohort pace (this isn't a scheduled
// course). `subsections` labels come directly from each folder's own
// _category_.json, not re-typed by hand.
...
export const TOTAL_PAGES = SECTION_ORDER.reduce((sum, key) => sum + SECTION_META[key].pageCount, 0);
