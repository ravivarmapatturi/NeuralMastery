...
const DATASET_OPTIONS = Object.entries(DATASETS).map(([value, d]) => ({ value, label: d.label }));
...
function NeuralNetworkPlaygroundInner() {
  ...
  useEffect(() => {
    draw();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [epoch, size, dataset, activation, layers, neurons, t.mode]);
  ...
  return (
    <VizCard
      ...
      footer="A real, hand-written forward/backward pass -- no TensorFlow.js -- training live in your browser on whichever toy dataset and architecture you pick."
    >
      <ControlRow>
        <PillSelect label="Dataset" value={dataset} onChange={setDataset} options={DATASET_OPTIONS} />
        <PillSelect label="Activation" value={activation} onChange={setActivation} options={ACTIVATION_OPTIONS} />
      </ControlRow>
      <ControlRow>
      ...
      <div ref={wrapRef} style={{ width: '100%', display: 'flex', gap: SPACING.md, flexWrap: 'wrap' }}>
        ...
        <div style={{ flex: 1, minWidth: 160 }}>
          ...
          <svg width="100%" height={80} viewBox="0 0 160 80" preserveAspectRatio="none">
            <polyline
              points={lossHistory
                .map((l, i) => `${(i / Math.max(1, lossHistory.length - 1)) * 160},${80 - (l / maxLoss) * 76}`)
                .join(' ')}
              fill="none"
          ...
          <div style={{ fontSize: 13, color: t.textSecondary, marginTop: SPACING.sm }}>
            ...
            <div style={{ marginTop: 6 }}>
              Shape: 2 → {Array(layers).fill(neurons).join(' → ')} → 1
            </div>
          </div>
        </div>
      </div>
...
export default function NeuralNetworkPlayground(props) {
  return (
    <BrowserOnly fallback={<VizCard title="Neural Network Playground">Loading…</VizCard>}>
      {() => <NeuralNetworkPlaygroundInner {...props} />}
    </BrowserOnly>
  );
}
