...
import { Slider, PillSelect, ControlRow } from './primitives/VizControls';
import { chunkCorpus, retrieve, rerank } from './lib/rag';
...
function buildFlow(t, rerankerOn) {
...
function RagPipelineSimulatorInner() {
  ...
  return (
    <VizCard
      ...
      footer="Real chunking, real bag-of-words cosine-similarity retrieval, and a real (if simplified) reranking pass -- type any question about the topics covered in the demo corpus (linear regression, gradient descent, overfitting, decision trees, CNNs, attention, KV cache, RAG itself) and watch retrieval actually respond."
    >
      <div style={{ marginBottom: SPACING.sm }}>
        <label style={{ display: 'block', fontSize: 14, color: t.textSecondary, marginBottom: 6 }}>
          Question
        </label>
        <input
          type="text"
      ...
      <ControlRow>
        <PillSelect label="Chunk size" value={chunkSize} onChange={setChunkSize} options={CHUNK_SIZE_OPTIONS} />
          ...
          {results.map((r, i) => (
            ...
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', color: t.accentPrimary, fontWeight: 700, marginBottom: 4 }}>
                <span>#{i + 1}</span>
                <span>{((r.rerankScore ?? r.score) * 100).toFixed(1)}% match</span>
              </div>
...
export default function RagPipelineSimulator(props) {
  return (
    <BrowserOnly fallback={<VizCard title="RAG Pipeline Simulator">Loading…</VizCard>}>
      {() => <RagPipelineSimulatorInner {...props} />}
    </BrowserOnly>
  );
}
