...
import { VOCAB, CATEGORIES, nearestNeighbors, analogy, pca2D } from './lib/embeddings';
...
function EmbeddingSpaceExplorerInner() {
  ...
  return (
    <VizCard
      ...
      footer="Hand-authored, structured demo vectors (no trained model here) -- but cosine similarity, nearest-neighbor lookup, the A - B + C word-vector analogy, and the 2D projection below are all real math, computed from scratch, live."
    >
      <div style={{ display: 'flex', gap: SPACING.md, flexWrap: 'wrap' }}>
              ...
              return (
                <g key={p.word} onClick={() => setSelected(p.word)} style={{ cursor: 'pointer' }}>
                  ...
                  <text
                    ...
                    fill={isSelected ? t.textPrimary : t.textMuted}
                  >
                    {p.word}
                  </text>
                </g>
        ...
        <div style={{ flex: 1, minWidth: 240 }}>
          ...
          <div style={{ fontSize: 13, fontWeight: 700, color: t.textPrimary, marginTop: SPACING.sm, marginBottom: 6 }}>
            Analogy: A − B + C ≈ ?
          </div>
...
export default function EmbeddingSpaceExplorer(props) {
  return (
    <BrowserOnly fallback={<VizCard title="Embedding Space Explorer">Loading…</VizCard>}>
      {() => <EmbeddingSpaceExplorerInner {...props} />}
    </BrowserOnly>
  );
}
