// Small, dependency-free toy dataset generators for the Neural Network
// Playground -- the same four classic 2D classification shapes every
// neural-network-playground tool uses (XOR, circles, moons, blobs), each
// returning { X: [[x, y], ...], y: [0 | 1, ...] } scaled to roughly [-1, 1].
...
export function makeXOR(n = 200, seed = 1) {
...
export function makeCircles(n = 200, seed = 1) {
  const rand = mulberry32(seed);
  const X = [];
  const y = [];
  for (let i = 0; i < n; i++) {
...
export function makeMoons(n = 200, seed = 1) {
  const rand = mulberry32(seed);
  const X = [];
  const y = [];
  for (let i = 0; i < n; i++) {
...
export function makeBlobs(n = 200, seed = 1) {
  const rand = mulberry32(seed);
  const X = [];
  const y = [];
  const centers = [
