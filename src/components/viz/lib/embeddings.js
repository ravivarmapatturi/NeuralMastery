...
// Each word's vector = a one-hot category direction + a gender-axis
// component (used only by the "people"/"royalty" categories, everything
// else is 0) + two small per-word jitter dimensions, so words in the same
// category cluster tightly but aren't literally identical.
...
function embedWord(word, category, gender) {
  const rand = mulberry32(hashString(word));
  const vec = CATEGORIES.map((c) => (c === category ? CATEGORY_SCALE : 0));
...
export const VOCAB = WORDS.map(([word, category, gender]) => ({
  word,
  category,
...
export function nearestNeighbors(word, k = 5) {
...
export function analogy(a, b, c, k = 3) {
...
function powerIteration(M, iterations = 60, seed = 1) {
...
function deflate(M, v, eigenvalue) {
  const d = M.length;
