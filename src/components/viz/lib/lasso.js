...
// correlated-feature dataset as ridge.js so the two pages' regularization
// paths are directly, honestly comparable -- same data, same lambda range.

export { generateData, FEATURE_NAMES, TRUE_W, TRUE_B } from './ridge.js';
...
export function lassoSolve(rows, lambda, epochs = 150) {
  const n = rows.length;
...
export function regularizationPath(rows, steps = 30) {
  const path = [];
