...
/** Fixed-size chunking by word count, matching chunkSize -- the same tradeoff rag.md describes: too small loses context, too large dilutes specificity. */
export function chunkCorpus(chunkSize) {
  const chunks = [];
...
export function retrieve(query, chunks, topK) {
  const qVec = bagOfWords(query);
  const scored = chunks.map((c) => ({ ...c, score: cosineSim(qVec, bagOfWords(c.text)) }));
 ...
 * coarser first pass -- see rag.md's re-ranking section.
 */
