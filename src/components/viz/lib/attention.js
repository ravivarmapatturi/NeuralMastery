...
// BPE (see foundation-model-internals.md for the real thing) -- adding a
// full BPE vocabulary/merge table would be the single biggest bundle-size
// cost of this component for a detail that isn't what this visualization is
// actually teaching (attention mechanics, not tokenization specifically).
// Embeddings and Q/K/V projection weights are deterministically seeded
// pseudo-random vectors/matrices (not trained) -- real enough to show the
// actual QKV -> scores -> softmax -> weighted-sum computation with real
// numbers, not real enough to claim any linguistic meaning in the result.
...
function randMatrix(rows, cols, seed) {
 ...
 */
export function runAttention(tokens, head) {
  ...
  const output = weights.map((w) =>
    V[0].map((_, d) => w.reduce((s, wij, j) => s + wij * V[j][d], 0))
  );
