...
function makeStepper(optimizer, lr) {
...
const DEFAULT_LR = { sgd: 0.15, momentum: 0.1, adam: 0.4 };

function GradientDescentExplorerInner({ defaultOptimizer = 'adam' }) {
  const t = useVizTokens();
  const [optimizer, setOptimizer] = useState(defaultOptimizer);
  const [lr, setLr] = useState(DEFAULT_LR[defaultOptimizer]);
  const [start, setStart] = useState({ x: -4, y: 2 });
  const [path, setPath] = useState([{ x: -4, y: 2 }]);
  const [playing, setPlaying] = useState(false);
  const stateRef = useRef({ x: -4, y: 2 });
  ...
  return (
    <VizCard
      ...
      footer="Click anywhere on the bowl to set a starting point, pick an optimizer, and watch it descend -- the exact update rules from Optimizers, In Full Depth, running live."
    >
      <ControlRow>
        <PillSelect label="Optimizer" value={optimizer} onChange={setOptimizer} options={OPTIMIZERS} />
      ...
      <VizCanvas aspect={9 / 6} minHeight={320} maxHeight={420}>
        {({ width, height }) => {
          const margin = 24;
          ...
          return (
            ...
            >
              ...
              <circle
                ...
                cy={yScale(last.y)}
                r={6}
                fill={converged ? t.accentPrimary : t.accentWarn}
              />
            </svg>
          );
        }}
      ...
      <div style={{ display: 'flex', gap: SPACING.md, fontSize: 13, color: t.textSecondary, marginTop: 4 }}>
        ...
        <span style={{ color: converged ? t.accentPrimary : t.textSecondary, fontWeight: converged ? 700 : 400 }}>
...
export default function GradientDescentExplorer(props) {
  return (
    <BrowserOnly fallback={<VizCard title="Gradient Descent Explorer">Loading…</VizCard>}>
      {() => <GradientDescentExplorerInner {...props} />}
    </BrowserOnly>
  );
}
