...
import { PillSelect } from './primitives/VizControls';
import { tokenize, makeHead, runAttention } from './lib/attention';
...
function AttentionStepThroughInner() {
  ...
  return (
    <VizCard
      ...
      footer="Real Q·Kᵀ/√d_k → softmax → weighted-sum-of-V math, computed on deterministic demo embeddings (see the component note for exactly what's simplified and why)."
    >
      <div style={{ marginBottom: SPACING.sm }}>
        <label style={{ display: 'block', fontSize: 14, color: t.textSecondary, marginBottom: 6 }}>
          Sentence (up to 12 tokens)
        </label>
        <input
          type="text"
          value={text}
      ...
      {result && (
        <div style={{ display: 'flex', gap: SPACING.lg, flexWrap: 'wrap', marginTop: SPACING.md }}>
          ...
          <div style={{ flex: 1, minWidth: 200 }}>
            <div style={{ fontSize: 13, color: t.textSecondary, marginBottom: 8 }}>
              Attention from <strong style={{ color: t.textPrimary }}>&ldquo;{tokens[focusRow]}&rdquo;</strong> to every
              token:
            </div>
            {tokens.map((tok, j) => {
              ...
              return (
                <div key={j} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                  ...
                  <div style={{ width: 42, fontSize: 12, color: t.textSecondary, fontVariantNumeric: 'tabular-nums' }}>
                    {(w * 100).toFixed(0)}%
                  </div>
                </div>
              );
...
export default function AttentionStepThrough(props) {
  return (
    <BrowserOnly fallback={<VizCard title="Attention Step-Through">Loading…</VizCard>}>
      {() => <AttentionStepThroughInner {...props} />}
    </BrowserOnly>
  );
}
