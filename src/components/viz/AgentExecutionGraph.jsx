...
import { PillSelect, VizButton, ControlRow } from './primitives/VizControls';
import { SCENARIOS, STEP_NODE } from './lib/agentTrace';
...
function buildFlow(t, activeNodeId) {
  const baseNode = (id, label, pos) => {
...
function AgentExecutionGraphInner() {
  ...
  useEffect(() => {
    ...
    timerRef.current = setInterval(() => {
      setStepIndex((i) => {
        if (i >= steps.length - 1) {
          return i; // hold on the final Answer instead of looping silently
        }
        return i + 1;
      });
    }, 1400);
  ...
  return (
    <VizCard
      ...
      footer="A scripted, representative ReAct trace -- not a live model call -- so the shape of the reason / act / observe loop is something you can step through and click, not just read about."
    >
      <ControlRow>
        <PillSelect label="Scenario" value={scenarioKey} onChange={changeScenario} options={SCENARIO_OPTIONS} />
        ...
        <div style={{ flex: 1, minWidth: 240 }}>
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', color: t.accentPrimary, fontWeight: 700, marginBottom: 4 }}>
              <span>Step {stepIndex + 1} / {steps.length} -- {currentStep.title}</span>
            </div>
            <div style={{ color: t.textPrimary, marginBottom: 6, fontFamily: currentStep.type === 'action' ? 'monospace' : FONT_FAMILY }}>
              {currentStep.detail}
            </div>
...
export default function AgentExecutionGraph(props) {
  return (
    <BrowserOnly fallback={<VizCard title="Agent Execution Graph">Loading…</VizCard>}>
