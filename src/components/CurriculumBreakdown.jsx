...
import { SECTION_META, SECTION_ORDER, timeEstimate, TOTAL_PAGES } from '@site/src/data/sectionMeta';
  ...
  return (
    <div>
      <div style={{ display: 'flex', gap: 32, flexWrap: 'wrap', margin: '8px 0 28px' }}>
        {STATS.map((s) => (
          <div key={s.label}>
            <div style={{ fontSize: 28, fontWeight: 800, color: 'var(--ifm-color-primary)' }}>{s.value}</div>
            <div style={{ fontSize: 13, color: 'var(--ifm-color-emphasis-600)' }}>{s.label}</div>
          </div>
        ))}
      </div>
        ...
        {SECTION_ORDER.map((key) => {
          ...
          return (
            ...
            >
              ...
              <p style={{ fontSize: 13.5, color: 'var(--ifm-color-emphasis-700)', margin: '0 0 10px' }}>{meta.description}</p>
              <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', fontSize: 12, color: 'var(--ifm-color-emphasis-600)', marginBottom: 12 }}>
