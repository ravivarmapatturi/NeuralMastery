...
The same scrutiny applies to training/fine-tuning data: a dataset scraped or aggregated from the web can carry mixed, unclear, or restrictive licensing on its constituent parts even when the dataset itself is published as "open" — training a commercial model on data with unclear provenance is a genuine legal risk, not just an ethical nicety, and is exactly the kind of thing [Data Engineering & Versioning](./data-engineering-and-versioning.md)'s provenance tracking exists to make auditable after the fact.
...
- **PII (Personally Identifiable Information)**: detecting, redacting, and controlling access to personal data flowing through training data, logs, and traces — see [Security & Reproducibility — PII Protection](./security-and-reproducibility.md) for the technical controls.
...
- **HIPAA** (U.S. healthcare): covered in depth for the healthcare-specific case in [Healthcare AI — Privacy and HIPAA](../domain-applications/healthcare-ai.md#privacy-and-hipaa) — the general pattern (access controls scoped to legitimate need, audit logging, careful third-party data-processing agreements) generalizes to other regulated-data domains beyond healthcare specifically.

## AI Governance

Organization-level policy and process for how AI systems get built, approved, and monitored — covered as part of the platform-engineering view in [MLOps → AI Platform Engineering — Governance](./mlops-architecture-and-roadmap.md#from-mlops-to-ai-platform-engineering): which models are approved for which use cases, who has authority to approve a production deployment, and how policy gets enforced technically (via the [CI/CD gates](./cicd-and-ml-cicd.md) covered earlier) rather than remaining a document nobody actually checks against before shipping.
