...
- **Distributed locks**: when multiple workers/processes might operate on the same resource concurrently (two training jobs writing to the same checkpoint location, two workers processing the same queued item), a distributed lock ensures only one actually proceeds at a time — implemented via a coordination service (the same kind of majority-consensus mechanism as [CS Fundamentals — Consensus](../cs-fundamentals/networking-and-distributed-systems.md#consensus-raftpaxos-intuition-only)) rather than assuming single-process, single-machine execution.
