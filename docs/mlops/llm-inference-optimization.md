...
The techniques that make the engines from [LLM Inference Engines](./llm-inference-engines.md) fast. Almost all of them exist to solve one of two problems: autoregressive generation is memory-bandwidth-bound, or the KV cache is too big.
...
- **Parallelism for serving**: tensor and pipeline parallelism (see [GPU/AI Infrastructure & Distributed Training](./gpu-and-distributed-training.md)) apply to inference too — splitting a model too large for one GPU across several, specifically to serve it, not just to train it.
...
Next: [LLM Hosting & Serving Patterns](./llm-hosting-and-serving-patterns.md) — where these engines actually run, and how a fine-tuned model gets from a training job to a served endpoint.
