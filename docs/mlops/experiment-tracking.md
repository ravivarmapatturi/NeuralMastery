...
- **Data**: which version of which dataset (see [Data Engineering & Versioning](./data-engineering-and-versioning.md))?
...
- **MLflow Model Registry**: a central store for model versions with stage transitions (Staging → Production → Archived) and lineage back to the run that produced each version — see [Feature Stores & Model Registry](./feature-stores-and-model-registry.md).
...
Next: [Pipeline Orchestration](./pipeline-orchestration.md) — once individual experiments are tracked, the next problem is stitching the steps around them into a repeatable, scheduled pipeline.
