...
- **Expert parallelism**: specific to Mixture-of-Experts models (see [Foundation Model Internals — MoE](../llms-genai/foundation-model-internals.md#mixture-of-experts-moe)) — different experts live on different GPUs, and each token's activations get routed to whichever GPU holds the expert its router selected, rather than every GPU holding a full copy of every expert.
