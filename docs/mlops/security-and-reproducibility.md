...
- **IAM**: identity and access management — who, and what service, can do what (see [Cloud Computing for ML](./cloud-computing.md)). Over-permissioned roles are the single most common source of real security incidents, not exotic exploits.
...
- **API auth/RBAC**: every serving endpoint (see [APIs & Model Serving](./model-serving.md), [LLM Hosting & Serving Patterns](./llm-hosting-and-serving-patterns.md)) needs authentication and role-based access control — not every caller should have access to every model or every capability.
- **TLS**: encrypt traffic in transit, always — between users and the API gateway, and ideally between internal services too.
- **Container/dependency scanning**: scan images and dependency trees for known CVEs as part of CI (see [Containers](./containers.md), [CI/CD & ML CI/CD](./cicd-and-ml-cicd.md)) — catching a vulnerable base image or package before it ships, not after.
...
- **Excessive agency**: giving an LLM-based agent (see [Agents](../agents/roadmap.md)) more real-world capability (API calls, file writes, financial transactions) than a given task actually requires — the fix is the same principle as least-privilege IAM, applied to what tools an agent is allowed to invoke and under what confirmation requirements.
- **PII protection**: detecting and redacting personally identifiable information both in data used for training/fine-tuning and in logs/traces captured during serving (see [Observability](./observability.md)) — logging full prompts and completions by default is a common, easily-avoided way to accidentally create a PII exposure.
...
- **Data**: the exact dataset version (see [Data Engineering & Versioning](./data-engineering-and-versioning.md)).
- **Model**: the exact architecture and starting weights (for fine-tuning).
- **Params**: every hyperparameter, logged (see [Experiment Tracking](./experiment-tracking.md)).
- **Environment**: exact package versions, ideally captured as a container image (see [Containers](./containers.md)), not just a `requirements.txt`.
...
Next: [The Full MLOps Architecture, Priority Stack & Learning Path](./mlops-architecture-and-roadmap.md) — putting every page in this section together into one end-to-end picture.
