...
- **Training platform**: the [pipeline orchestration](./pipeline-orchestration.md) + [experiment tracking](./experiment-tracking.md) + [GPU/distributed training](./gpu-and-distributed-training.md) layers, offered as a shared service — a data scientist submits a training job without personally provisioning GPUs, configuring FSDP, or standing up an MLflow server.
- **Inference platform**: the [LLM inference engines](./llm-inference-engines.md) through [hosting & serving patterns](./llm-hosting-and-serving-patterns.md) layers, similarly centralized — teams deploy a model to a shared, already-optimized serving platform instead of each reinventing vLLM deployment and autoscaling from scratch.
- **Experiment platform** and **evaluation platform**: shared, centrally-maintained instances of [experiment tracking](./experiment-tracking.md) and [AI Evaluation](../ai-evaluation/roadmap.md) tooling — the value of centralizing these specifically is *comparability*: experiments and evaluations run the same way, on the same infrastructure, are actually comparable across teams, where independently-run ad hoc evaluations often aren't.
- **GPU orchestration**: centralized scheduling and cost allocation of a shared GPU fleet across teams (see [Kubernetes — GPU scheduling](./kubernetes.md#why-this-matters-for-ml-specifically)) — the platform team's job is making expensive, scarce GPU capacity fairly and efficiently shared, rather than every team over-provisioning its own idle capacity out of uncertainty about what it'll get.
...
- **Governance**: organization-wide policy enforcement — which models are approved for production use, which data classifications can feed which pipelines, who can approve a production deployment — layered on top of the technical [CI/CD gates](./cicd-and-ml-cicd.md) as an organizational control, not just a technical one.
- **Lineage**: being able to answer, for any production prediction, the full chain that produced it — which model version, trained on which data version, with which code version, deployed via which pipeline run — stitching together [data versioning](./data-engineering-and-versioning.md), [experiment tracking](./experiment-tracking.md), and the [model registry](./feature-stores-and-model-registry.md) into one traceable chain, essential for debugging, auditing, and [legal/compliance](../mlops/roadmap.md) requirements alike.
