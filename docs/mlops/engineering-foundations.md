...
- **API design**: if your model is served behind an API (see [APIs & Model Serving](./model-serving.md)), the contract (request/response schema, error format, versioning) is a design decision made deliberately, not whatever `FastAPI` happens to auto-generate from your first draft.
...
- **Data validation tests**: assert incoming data matches expected schema, ranges, and types *before* it reaches the model — see [Data Engineering & Versioning](./data-engineering-and-versioning.md) for the pipeline stage this belongs to.
...
Next: [Data Engineering & Versioning](./data-engineering-and-versioning.md) — MLOps starts with data, and data needs the same engineering discipline as code.
