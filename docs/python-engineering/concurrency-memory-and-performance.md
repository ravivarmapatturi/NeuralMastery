...
[CS Fundamentals](../cs-fundamentals/operating-systems-and-concurrency.md) covered concurrency/parallelism and the GIL conceptually. This page is the Python-specific mechanics: which tool to actually reach for, how CPython manages memory, and where Python performance really goes in ML code.
...
Given the GIL (see [CS Fundamentals](../cs-fundamentals/operating-systems-and-concurrency.md#pythons-gil-and-why-it-matters-for-ml-specifically)):
...
- **`py-spy`**: a sampling profiler that can attach to an *already-running* Python process (including in production) with negligible overhead, producing flame graphs (see [CS Fundamentals](../cs-fundamentals/software-engineering-practices.md#profiling)) without needing to restart anything with instrumentation enabled.
...
- **The GPU-specific version of this same lesson**: launching many small CUDA operations (the GPU equivalent of a Python `for` loop) pays per-launch overhead repeatedly, instead of one larger fused operation — see [GPU/AI Infrastructure & Distributed Training](../mlops/gpu-and-distributed-training.md) and [LLM Inference Optimization](../mlops/llm-inference-optimization.md#other-core-optimizations)'s discussion of kernel fusion and CUDA graphs, which is this exact same vectorization principle one level down the stack.

Next: [Packaging, Testing & Tooling](./packaging-testing-and-tooling.md) — making code like this shippable and maintainable, not just fast.
