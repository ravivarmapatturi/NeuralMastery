...
[PCA and its relatives](./pca-svd.md) all optimize for *variance*. This page covers three methods with a different goal entirely: ICA separates mixed signals by *statistical independence*, while t-SNE and UMAP optimize purely to make high-dimensional data *visualizable*, with no interest in variance or interpretable axes at all.
...
**Why independence, and why it works**: ICA relies on the Central Limit Theorem (see [Probability & Statistics](../mathematics-for-ai/probability-statistics.md#central-limit-theorem)) in reverse — a mixture of independent signals tends to look *more* Gaussian than any of the original signals alone (sums of independent random variables trend toward Gaussian). So ICA searches for a transformation that makes the resulting components as **non-Gaussian** as possible, on the logic that this is exactly what un-mixes the original signals back out.
...
Built for exactly one job: taking high-dimensional data (like embeddings — see [Foundation Model Internals](../llms-genai/foundation-model-internals.md#embeddings)) and projecting it into 2D/3D **purely for visualization**, preserving local neighborhood structure as faithfully as possible.
...
![A 2D embedding showing three well-separated categories — the goal of any t-SNE/UMAP/PCA visualization](./img/embedding-2d.png)

This is exactly the kind of plot you'd generate to sanity-check a trained embedding model (see [RAG — Choosing an Embedding Model](../llms-genai/rag.md#choosing-an-embedding-model)): project a sample of embeddings to 2D and visually confirm that semantically different categories actually separate, before trusting the embedding space for retrieval.
...
This completes dimensionality reduction: [PCA/Kernel PCA/Truncated SVD](./pca-svd.md) → ICA/t-SNE/UMAP. Next: [Anomaly Detection](./unsupervised-learning.md#anomaly-detection) and the remaining unsupervised methods — association rule mining and autoencoders.
