...
- **Imbalanced classes**: when one class vastly outnumbers another (e.g. 1% fraud rate). A naive model can get 99% accuracy by always predicting "not fraud" — which is useless. Addressed with resampling, class weighting, or better metrics (see [Model Evaluation & Metrics](./model-evaluation-metrics.md)).
...
Next: [Supervised Learning](./supervised-learning.md) — the algorithms that turn this workflow into predictions.
