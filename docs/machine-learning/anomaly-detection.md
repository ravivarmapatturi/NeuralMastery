...
![Anomaly detection: a normal cluster with a handful of clear outliers flagged](./img/anomaly-detection.png)
...
A density-based approach, conceptually close to [DBSCAN](./dbscan-hdbscan.md#dbscan-density-based-spatial-clustering-of-applications-with-noise): compare each point's local density to the local density of its neighbors. A point sitting in a much sparser neighborhood than its neighbors' neighborhoods gets a high outlier score.

**Why "local" matters specifically**: a single global density threshold (like DBSCAN's fixed $\epsilon$) fails when different regions of the data have genuinely different natural densities — a point that's sparse relative to a dense region might be entirely normal relative to a naturally sparser region elsewhere in the same dataset. LOF compares each point only to its *own* neighborhood's typical density, making it far more sensitive to local anomalies that a global threshold would miss entirely — directly the same varying-density problem that motivated [HDBSCAN](./dbscan-hdbscan.md#hdbscan-hierarchical-dbscan) over plain DBSCAN.

## One-Class SVM

Adapts [SVM](./support-vector-machines.md) to a setting with only one class (normal data, little or no labeled anomalies): instead of finding a boundary *between* two classes, it finds a boundary that separates the normal data from the origin (or, in the more common variant, fits the smallest possible enclosing region around the normal data) in the kernel-transformed feature space (see [SVM — The Kernel Trick](./support-vector-machines.md#the-kernel-trick)).
...
Covered in depth in [Autoencoders](../deep-learning/autoencoders.md#the-autoencoder-architecture): train a reconstruction model on normal data only; at inference, high reconstruction error signals an anomaly, since the model never learned to compress and reconstruct that kind of input well. The deep-learning analog of the same "model what's normal, flag what doesn't fit" strategy as every method above.
...
**General rule of thumb**: Isolation Forest is the strong default for tabular data at any real scale; LOF when you specifically suspect varying-density regions; One-Class SVM for smaller, well-understood feature spaces; autoencoders when the data is high-dimensional/unstructured (images, sequences) where deep learning already has an architectural advantage (see [Deep Learning](../deep-learning/roadmap.md)).

This completes association rules and anomaly detection. Next: [Topic Modeling: LDA & BERTopic](./topic-modeling.md) — unsupervised structure discovery specifically for text.
