...
**The algorithm** (already implemented once, informally, inside [Random Forest's](./random-forest.md) cousin methods — here it's the main event):

1. Choose $k$ (the number of clusters) and initialize $k$ centroids — commonly by picking $k$ random data points.
2. **Assign**: put each point into the cluster of its nearest centroid (by Euclidean distance — see [Linear Algebra](../mathematics-for-ai/linear-algebra.md)).
3. **Update**: move each centroid to the mean position of the points now assigned to it.
4. Repeat steps 2-3 until assignments stop changing.

![K-Means clustering: three clusters found automatically, with centroids marked](./img/kmeans-clusters.png)
...
![K-Means fails on non-convex "two moons" data; density/graph-based methods respect the true shape](./img/clustering-shape-comparison.png)
...
![A hierarchical clustering dendrogram — cutting at any height gives a different number of clusters](./img/dendrogram.png)
...
Next: [DBSCAN & HDBSCAN](./dbscan-hdbscan.md) — clustering by density instead of distance-to-centroid, which handles non-convex shapes natively.
