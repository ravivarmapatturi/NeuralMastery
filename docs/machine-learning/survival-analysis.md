...
![Kaplan-Meier survival curves for two groups, with censoring marks](./img/survival-curve.png)
...
To test whether two Kaplan-Meier curves are *significantly* different (not just visibly different, per [Hypothesis Testing](../mathematics-for-ai/probability-statistics.md#hypothesis-testing--significance)), the **log-rank test** compares the observed number of events in each group, at each event time, against the number *expected* if both groups actually had identical survival — summed into a single chi-squared test statistic across all event times. This is the standard significance test reported alongside a Kaplan-Meier plot in a clinical trial or an A/B-style churn comparison.
...
Next: [Recommender Systems](./recommender-systems.md) — another "specialized" supervised problem, predicting preference instead of time.
