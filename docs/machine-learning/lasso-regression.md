...
Same hypothesis as [Linear Regression](./linear-regression.md) — $\hat{y} = \mathbf{w}^T\mathbf{x} + b$ — with a different penalty than Ridge added to the cost function. "Lasso" stands for **L**east **A**bsolute **S**hrinkage and **S**election **O**perator — the name is a description of exactly what it does.

## The Lasso Cost Function

$$J_{\text{lasso}}(\mathbf{w}, b) = \frac{1}{n}\sum_{i=1}^n \left(\hat{y}^{(i)} - y^{(i)}\right)^2 + \lambda \sum_{j=1}^d |w_j|$$

The only change from [Ridge](./ridge-regression.md#the-ridge-cost-function) is the penalty term: $\lambda \sum |w_j|$ — the **L1 norm** of the weights (sum of absolute values, see [Linear Algebra — Norms](../mathematics-for-ai/linear-algebra.md)) instead of the L2 norm (sum of squares).
...
Compare directly against [Ridge's path](./ridge-regression.md#the-regularization-path) — same synthetic data, same range of $\lambda$:
...
Next: [Elastic Net](./elastic-net.md) — combining both penalties to get Ridge's stability with Lasso's feature selection.
