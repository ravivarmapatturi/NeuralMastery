...
- **Mini-Batch Gradient Descent**: the practical universal default — a small batch (32-512 examples) per step. Balances gradient accuracy against update frequency, and maps naturally onto GPU parallelism (see [Algorithms & Data Structures](../mathematics-for-ai/algorithms-data-structures.md)). When people say "SGD" in a deep learning context, they almost always mean this.
...
- **Classical ML / convex problems** (see [Linear Regression](../machine-learning/linear-regression.md#7-closed-form-vs-gradient-descent)): plain SGD or even the closed-form solution, since there's no messy non-convex surface to navigate.
...
Next: [Weight Initialization, Regularization & LR Scheduling](./initialization-regularization-scheduling.md) — the remaining training-loop decisions that determine whether an optimizer like this actually converges well.
