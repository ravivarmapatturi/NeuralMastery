...
- **Cross-entropy**: the standard loss for classification — measures how far the predicted probability distribution is from the true one (see [Probability & Statistics](../mathematics-for-ai/probability-statistics.md)). This is also the loss behind next-token prediction in every LLM.
...
- Standard L1/L2 weight regularization still applies (see [Model Evaluation & Metrics](../machine-learning/model-evaluation-metrics.md)), though it's used more sparingly in very large models where data volume itself is often the main regularizer.
...
- **Warmup + decay**: start small, ramp up, then decay — covered in [Calculus & Optimization](../mathematics-for-ai/calculus-optimization.md); essential for training stability in Transformers specifically.
...
Next: [Convolutional Neural Networks](./cnns.md) — the architecture that made deep learning work for images.
