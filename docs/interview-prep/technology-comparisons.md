...
Covered individually in [LLM Inference Engines](../mlops/llm-inference-engines.md) — the decision in one pass:
...
- **LangGraph**: purpose-built for representing agents as an explicit state graph — nodes and edges, with conditional routing between them (see [Agent Architectures — Supervisor Pattern](../agents/agent-architectures.md#supervisor-pattern)) — the right choice once an agent's control flow is genuinely non-linear (loops, conditional branches, multiple agents handing off to each other) and you need to reason about and debug that flow explicitly, which a simpler chain abstraction doesn't expose well.

## RAG vs. Fine-Tuning

The single most common "which should I use" question in applied LLM work — see [Prompt Engineering](../llms-genai/prompt-engineering.md) and [Training Pipeline](../llms-genai/training-pipeline.md) for each individually:
...
Covered in [Training Pipeline — PEFT](../llms-genai/training-pipeline.md#parameter-efficient-fine-tuning-peft):
...
- **SQL (relational)**: structured data with well-defined relationships, needing exact queries, transactions, and strong consistency guarantees — the default for transactional/operational data (see [Databases — Relational](../databases/relational/overview.md)).
- **Vector database**: semantic similarity search over embeddings — the right tool when the query is "find things *similar in meaning* to this," not an exact match (see [Databases — Vector](../databases/vector/overview.md)), the backbone of RAG retrieval.
- **Graph database**: data where the *relationships* between entities are the primary thing being queried — multi-hop traversals ("friends of friends who also..."), not just storing connected data in a relational schema (see [Databases — Graph](../databases/graph/overview.md)) — the right choice when queries are naturally graph-shaped (GraphRAG, fraud rings, recommendation via shared connections) rather than forcing a graph-shaped query through relational joins, which gets slow and unwieldy past a few hops.
...
Covered in [GPU/AI Infrastructure & Distributed Training](../mlops/gpu-and-distributed-training.md):
...
- **PyTorch**: the dominant framework for the overwhelming majority of production ML and LLM work — eager-mode-first execution (easier to debug, more intuitive control flow), the largest ecosystem of pretrained models and libraries, and where essentially every tool covered in this site's [MLOps](../mlops/roadmap.md) and [Inference Engineering](../mlops/llm-inference-engines.md) coverage assumes you're starting from.
...
Next: [System Design Practice](./system-design-practice.md) — where these tradeoffs get applied inside a full system design interview, not just recited in isolation.
