...
Covered in full architectural depth in [Vision Architectures](../deep-learning/vision-architectures.md) — linked here for completeness of the taxonomy:

- **Classification**: assign one label to an entire image. The original CNN task ([CNNs](../deep-learning/cnns.md)'s LeNet-through-ConvNeXt lineage).
...
- **Text recognition**: given a cropped text region, decode the actual character sequence — historically CNN + RNN/CTC-loss pipelines (predict a character sequence without needing pre-segmented individual characters), increasingly Transformer-based sequence-to-sequence models that treat this as image-to-text generation directly (see [Attention & Transformers](../deep-learning/attention-transformers.md)).
- **Modern approach**: end-to-end Transformer-based OCR models (and general-purpose VLMs, see [Modern Vision & Multimodal](./modern-vision-and-multimodal.md)) increasingly fold detection and recognition into one model rather than a two-stage pipeline, especially for documents with complex layouts (tables, forms) where layout understanding matters as much as character recognition.
...
Next: [Modern Vision & Multimodal](./modern-vision-and-multimodal.md) — where vision meets language, video, and 3D.
