...
Interleaves reasoning steps with actions (tool calls) and observations: *think → act → observe → think → ...*. This is the foundational pattern behind most tool-using agents (see [Agents](../agents/roadmap.md)) — the model reasons about what it needs, calls a tool to get it, incorporates the result, and continues.
...
- **The "lost in the middle" problem**: models attend less reliably to information buried in the middle of a long context, even when it's technically within the context window — relevant to both prompting and RAG (see [RAG](./rag.md)).
...
Next: [Retrieval-Augmented Generation (RAG)](./rag.md) — giving a model access to knowledge beyond what's in its weights.
