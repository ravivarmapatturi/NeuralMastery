...
[Multi-agent systems](../multi-agent-systems.md) covers *architectural patterns* for coordinating multiple agents (orchestrator/sub-agent, peer-to-peer, etc.) — A2A is one concrete protocol for implementing the communication layer those patterns need, particularly relevant when the agents involved aren't all built and controlled by the same team, and can't simply share a Python process and call each other's functions directly.

Agent protocols section complete. Next: [Multi-Agent Systems](../multi-agent-systems.md) — the coordination patterns these protocols enable.
