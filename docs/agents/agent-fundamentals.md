...
- **Long-term memory**: information that needs to persist *across* sessions or beyond what fits in context — typically implemented as an external store (a database or vector store) that the agent can write to and query, effectively turning memory into a retrieval problem (see [RAG](../llms-genai/rag.md)).
...
Next: [Agent Architectures](./agent-architectures.md) — the concrete patterns for structuring the reasoning-and-acting loop.
